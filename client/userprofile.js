...
// Simulated posts (replace with DB fetch later)
for (let i = 1; i <= 6; i++) {
  const div = document.createElement("div");
  div.className = "post";
  div.innerText = `Post ${i}`;
  postsContainer.appendChild(div);
}

// Simulated videos (replace with DB fetch later)
for (let i = 1; i <= 3; i++) {
  const div = document.createElement("div");
  div.className = "post video";
  div.innerText = `Video ${i}`;
...
function loadProfile() {
  ...
  if (!profile) {
    const defaultProfile = {
      ...
      profilePic: "https://via.placeholder.com/150/0095f6/FFFFFF?text=IC"
    };
    localStorage.setItem("userProfile", JSON.stringify(defaultProfile));
    profile = defaultProfile;
  }

  document.getElementById("profileName").innerText = profile.firstName + " " + profile.surname;
  ...
  document.getElementById("profileBio").innerText = "Bio: " + profile.bio;

  if (profile.profilePic) {
    document.getElementById("profilePhoto").src = profile.profilePic;
  }

  document.getElementById("postCount").innerText = postsContainer.children.length;
  document.getElementById("videoCount").innerText = videosContainer.children.length;
}
...
tabButtons.forEach(button => {
  button.addEventListener("click", () => {
    ...
    document.getElementById(button.getAttribute("data-content") + "Container").classList.remove("hidden");
  });
});
...
document.getElementById("closeBioModalIcon").addEventListener("click", () => {
  document.getElementById("bioModal").classList.add("hidden");
});
